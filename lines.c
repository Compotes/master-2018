...
uint8_t line_calibration(void){
	if(chMBFetch(&line_commands, &msg, TIME_IMMEDIATE) == MSG_OK)
		return TRUE;
	return FALSE;
}

void send_line_calibration(void){
	chMBPost(&line_commands, MSG_OK, TIME_INFINITE);
}
...
THD_FUNCTION(LineThread, arg) {
	...
	while (1) {
		...
		if(line_calibration_timer < LINE_CALIBRATION_TIME){
			calibrate_lines();
...
THD_FUNCTION(LineSaveThread, arg) {
	...
	while (1) {
		...
		if(command == NO_LINE_DETECTED || command == LINE_CALIBRATION){
			line_state = command;
		} else if(command == LOAD_LINES_SPEEDS && line_state != NO_LINE_DETECTED){
			for(i = 0; i < NUMBER_OF_MOTORS; i++){
